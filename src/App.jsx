...
const App = () => {
  ...
  // ★ 音源ファイルをその場で読み込むための関数（これが抜けていました）
      ...
      snapshot.forEach(async (doc) => {
        ...
        if (!audioBuffersRef.current[doc.id] && data.downloadURL) {
          try {
            ...
            audioBuffersRef.current[doc.id] = await ctx.decodeAudioData(arrayBuffer);
          } catch (e) { console.error("Firebase Sound Load Error", e); }
        }
      });
      setSoundBank(prev => {
        const localOnes = prev.filter(s => s.id.startsWith('local-'));
        return [...localOnes, ...sounds];
      });
    });
  }, [user]);
  ...
  return (
    <div style={{ backgroundColor: '#09090b', minHeight: '100vh', color: '#e4e4e7', padding: '10px', fontFamily: 'sans-serif' }}>
      <div style={{ maxWidth: '800px', margin: '0 auto' }}>
        ...
        <header style={{ display: 'flex', flexWrap: 'wrap', gap: '10px', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px', background: '#18181b', padding: '10px', borderRadius: '15px', border: '1px solid #27272a' }}>
          <div style={{ display: 'flex', gap: '5px' }}>
            <button onClick={() => setAspectRatio('portrait')} style={{ background: aspectRatio === 'portrait' ? '#f97316' : '#27272a', border: 'none', padding: '6px 10px', borderRadius: '8px', color: 'white', fontSize: '11px' }}>📱 縦</button>
            <button onClick={() => setAspectRatio('landscape')} style={{ background: aspectRatio === 'landscape' ? '#f97316' : '#27272a', border: 'none', padding: '6px 10px', borderRadius: '8px', color: 'white', fontSize: '11px' }}>💻 横</button>
          </div>

          <div style={{ display: 'flex', gap: '5px' }}>
            {/* ★ ここに「動画読込」と「音読込」の2つのボタンを並べました */}
        ...
        <div style={{ display: 'flex', gap: '15px', alignItems: 'flex-start', justifyContent: 'center' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            <div style={{ 
              ...
              {/* ★ 文字の配置：inset 0 と flex center で完全にど真ん中に固定 */}
              <div style={{ 
                ...
                {!isPlaying ? (
                  ...
                  <p style={{ fontSize: `${currentScript.fontSize}px`, fontWeight: 'bold', textAlign: 'center', color: currentScript.textColor, textShadow: heavyStroke, whiteSpace: 'pre-wrap', wordBreak: 'break-all', margin: 0 }}>{displayText}</p>
                )}
              </div>
            </div>
          ...
          <div style={{ width: '55px', display: 'flex', flexDirection: 'column', gap: '15px', background: '#18181b', padding: '15px 5px', borderRadius: '15px', border: '1px solid #27272a', alignItems: 'center' }}>
            ...
            <div style={{ textAlign: 'center' }}><span style={{ fontSize: '7px' }}>SIZE</span><input type="range" min="10" max="150" value={currentScript.fontSize} onChange={(e) => setScripts(scripts.map(s => s.id === activeId ? { ...s, fontSize: parseInt(e.target.value) } : s))} style={{ writingMode: 'bt-lr', appearance: 'slider-vertical', width: '4px', height: '60px' }} /></div>
      ...
      {showColorPicker && (
        <div style={{ position: 'fixed', bottom: '80px', right: '20px', background: '#18181b', padding: '10px', borderRadius: '12px', border: '1px solid #f97316', zIndex: 100 }}>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '5px', maxWidth: '100px' }}>
            {PRESET_COLORS.map(c => <button key={c} onClick={() => setScripts(scripts.map(s => s.id === activeId ? { ...s, textColor: c } : s))} style={{ background: c, border: currentScript.textColor === c ? '2px solid white' : '1px solid #27272a', width: '18px', height: '18px', borderRadius: '50%' }} />)}
          </div>
        </div>
      )}
    </div>
